...
public class DNA  implements Serializable{
	...
	public void getStrand(byte[] strand, int[] availabilityBoundaries){
        ...
        for (int i = 0; i < availabilityBoundaries.length; i+=2){
            ...
            index += availabilityBoundaries[i+1] - availabilityBoundaries[i];
        }
    }
	...
	public void occupyDNA(Cell n, int proteinID, int position, int size){
		...
		int affinityPosition = position;

        //bad luck
        //need refactoring
        /*
		try {
			if (n.dbp[proteinID].pe.isLeftSlidingEvent){
				affinityPosition = position + size;
			} else if (n.dbp[proteinID].pe.isRightSlidingEvent){
				affinityPosition = position - n.dbp[proteinID].size;
			} else {
				affinityPosition = position;
			}
