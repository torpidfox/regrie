...
public class DNA implements Serializable {
    ...
    // boundaries
    public int[] openRegionBoundaries;//pairs of indices (bonded DNA open regions) of left (% 2 == 1) and right (% 2
    // == 0) boundaries
    ...
    public int[] getDnaOpenRegionsBoundaries(int[] availabilityBoundaries) {
        ...
        for (int i = 0; i < availabilityBoundaries.length; i += 2) {
            if (i == 0) {
                ...
                currentLength = availabilityBoundaries[i + 1] - availabilityBoundaries[i];
                indicesOfBoundaries.add(indicesOfBoundaries.get(i - 1) + currentLength);
            }
...
//					indices.add(i);
//					while (i < endPosition-1 & dna.chromatin[i] & (i < this.codingRegion.start || i > this
// .codingRegion.end)){
//						i++;
//					}
//					if (indices.get(indices.size() - 1) == i - 1){
...
//			}
     ...
     * This its followed by the name of the chromosome or strain and the position where this sequence was extracted, e
     * .g. MG1655:0..4639675
            ...
            for (int i = 0; i < TFspecies.length; i++) {
				...
				*/
                ...
                for (int j = 0; j < strand.length; j++) {
                    ...
                    //if(TFaffinitiesLR[i][j]!=Constants.NONE && (j < n.dna.codingRegion.start || j > n.dna
                    // .codingRegion.end) && n.dna.chromatin[j]){
...
//			debug printing
                ...
                for (int i = 0; i < n.tsg.ts.size(); i++) {
                    ...
                    if (direction != Constants.NONE && direction >= 0 && direction < TFdirections) {
                        for (int k = startRel; k < endRel; k++) {
                            ...
                            //System.out.println(n.tsg.ts.get(i).TFid+","+k+", "+direction  +":"+this.isTargetSite[n
                            // .tsg.ts.get(i).TFid][k][direction] );
                        }
                    } else {
                        //if there is no direction specified then mark all directions
                        for (int k = startRel; k < endRel; k++) {
                            for (int dir = 0; dir < TFdirections; dir++) {
                                ...
                                this.areTargetSites = true;
                                //System.out.println(n.tsg.ts.get(i).TFid+","+k+", "+dir  +":"+this.isTargetSite[n
                                // .tsg.ts.get(i).TFid][k][dir] );
                            }
                        }
                    }
                }
                    ...
                    for (int k = j; k < max; k++) {
                        if (k >= 0 && k < strand.length - TFsize[i]) {
                            if (printBindingEnergy) {
                                ...
                                //System.out.println(bufferTFaffinity[i][k][dir]+"; "+tfs[i]
                                // .specificWaitingMoveTime+"; "+ TFavgMoveRate[i][j][dir]);
                            }
                        }
    ...
    public int bindMolecule(Cell n, int proteinID, int position, int size, boolean checkOccupancy) {
        ...
        if (canBind == proteinID) {
            occupyDNA(n, proteinID, position, size, false);
    ...
    private int canBind(int proteinID, int position, int size, boolean checkOccupancy, Cell n) {
        ...
        } else if (canBind == Constants.REPRESSED) {
...
//			+ "at position " + position);
    ...
    public boolean unbindMolecule(Cell n, int proteinID, int position, int size) {
        ...
        if (position >= 0 || position <= this.strand.length - size) {
            ...
            freeDNA(n, position, size, position, false, false);
    ...
    private void recomputeTFAffinityLandscapeOnBinding(Cell n, int position, int size) {
        ...
        if (n.dna.getBoundMolecule(position) != Constants.NONE && n.dbp[n.dna.getBoundMolecule(position)].repressionState) {
            ...
            for (int i = 0; i < TFsize.length; i++) {
                start = Math.max(0,
                        position - TFsize[i] + 1 - n.TFspecies[n.dbp[n.dna.getBoundMolecule(position)].speciesID].repLenLeft);
                end =
                        Math.min(position + size + n.TFspecies[n.dbp[n.dna.getBoundMolecule(position)].speciesID].repLenRight, n.dna.strand.length);
            ...
            for (int i = 0; i < TFsize.length; i++) {
                ...
                if (!this.effectiveTFavailability[i][Math.max(0, start - 1)]) {
                    ...
                    if (buffer[0] != Constants.NONE) {
                        ...
                        Math.min(position + size + n.TFspecies[n.dbp[n.dna.getBoundMolecule(position)].speciesID].repLenRight, this.strand.length - 1);
                ...
                int maxPos =
                        strand.length - TFsize[i] - n.TFspecies[n.dbp[n.dna.getBoundMolecule(position)].speciesID].repLenRight;
                for (int j = start; j < end; j++) {
    ...
    public int slideRight(Cell n, int proteinID, int position, int proteinSize, int stepSize, boolean checkOccupancy) {
        ...
        if (canSlide == proteinID && checkOccupancy) {
            ...
            if (canSlide == Constants.NONE) {
                canSlide = proteinID;
            } else if (canSlide == Constants.REPRESSED
                    && n.dbp[proteinID].repressionState) {
                canSlide = proteinID;
            }
        }
        ...
        if (canSlide == proteinID) {
            int occSize = n.TFspecies[n.dbp[proteinID].speciesID].sizeTotal;

            if (position != Constants.NONE && n.dna.getBoundMolecule(position) != Constants.NONE) {
                if (n.dbp[n.dna.getBoundMolecule(position)].repressionState) {
                    //occSize += n.TFspecies[n.dbp[proteinID].speciesID].sizeTotal;
                }
            }
            //first to free otherwise repressors can't slide
            freeDNA(n, position, occSize, position, false, true);
            occupyDNA(n, proteinID, newPosition, occSize, false);
    ...
    public int slideLeft(Cell n, int proteinID, int position, int proteinSize, int stepSize, boolean checkOccupancy) {
        ...
        //AD: take into account boundaries of open regions on the DNA
        int leftBoundary = findLeftBoundary(n, position);
        ...
        if (canSlide == proteinID) {
            ...
            freeDNA(n, position + proteinSize - occSize, occSize, position, true, false);
            //n.tsg.updateTargetSiteStatistics();
            //n.tsg.updateTargetSiteStatistics(n.dna.isTargetSite[n.dbp[i].speciesID][position][n.dbp[i].getDirection()]
            occupyDNA(n, proteinID, position - stepSize, occSize, false);
    ...
    private void _repress(Cell n, int position, int size, int proteinID) {
        ...
        int end = Math.min(findRightBoundary(n, position),
                position + n.TFspecies[n.dbp[proteinID].speciesID].sizeTotal + n.TFspecies[n.dbp[proteinID].speciesID].repLenRight);
     ...
     * @param isSliding
     */
    public void occupyDNA(Cell n, int proteinID, int position, int size, boolean isSliding) {
        ...
        if (n.TFspecies[n.dbp[proteinID].speciesID].repressor && (_isRepression(n, position, proteinID))) {
    ...
    private void _freeRepressed(Cell n, int positionToFree, int size, int position) {
        int proteinID = n.dna.getBoundMolecule(position);
        n.dbp[proteinID].repressionState = false;
    ...
    private void _moveRepressionOnSlidingRight(Cell n, int positionToFree, int size, int position) {
        int proteinID = n.dna.getBoundMolecule(position);
        int leftBnd = findLeftBoundary(n, position);
        int toFreeStart = Math.max(leftBnd,
                position - n.TFspecies[n.dbp[proteinID].speciesID].repLenLeft);


        if (toFreeStart != leftBnd) {
            for (int i = toFreeStart; i < toFreeStart + size; i++)
                this.occupied[i] = Constants.NONE;
        }

        int rightBnd = findRightBoundary(n, position);
        int toOccupyStart = Math.min(rightBnd, position + size + n.TFspecies[n.dbp[proteinID].speciesID].repLenRight);

        if (toOccupyStart != rightBnd) {
            for (int i = toOccupyStart; i < toOccupyStart + size; i++)
                this.occupied[i] = Constants.REPRESSED;
        }
    }

    private void _moveRepressionOnSlidingLeft(Cell n, int positionToFree, int size, int position) {
        int proteinID = n.dna.getBoundMolecule(position);
        int rightBnd = findRightBoundary(n, position);
        int toFreeStart = Math.min(rightBnd,
                positionToFree + n.TFspecies[n.dbp[proteinID].speciesID].repLenRight - size);


        if (toFreeStart != rightBnd) {
            for (int i = toFreeStart; i < toFreeStart + size; i++)
        ...
        int leftBnd = findLeftBoundary(n, position);
        int toOccupyStart = Math.max(leftBnd,
                position - n.TFspecies[n.dbp[proteinID].speciesID].repLenRight - size);

        if (toOccupyStart != leftBnd) {
            for (int i = toOccupyStart; i < toOccupyStart + size; i++)
                if (this.occupied[i] == Constants.NONE)
                    this.occupied[i] = Constants.REPRESSED;
        }

        this.occupied[positionToFree] = Constants.NONE;
    }
     ...
     * @param size           the size of the protein
     * @param position       the position on the dna to bind to
     * @param isSlidiingLeft
     * @param isSlidingRight
     */
    public void freeDNA(Cell n, int positionToFree, int size,
                        int position, boolean isSlidingLeft, boolean isSlidingRight) {
        int boundMolecule = n.dna.getBoundMolecule(position);

        if (boundMolecule != Constants.NONE && n.dbp[boundMolecule].repressionState) {
//            if (isSlidingLeft) {
//                _moveRepressionOnSlidingLeft(n, positionToFree, size, position);
//            }
//            else if (isSlidingRight) {
//                _moveRepressionOnSlidingRight(n, positionToFree, size, position);
//            }
//            else {
    ...
    public int getBoundMolecule(int position) {
        return this.occupied[position] != Constants.NONE && this.occupied[position] != Constants.REPRESSED
    ...
    public void printOccupnacy(double i) throws IOException {
        BufferedWriter outputWriter = null;
        outputWriter = new BufferedWriter(new FileWriter("occupancy" + i + ".txt"));

        outputWriter.write(Arrays.toString(occupied));
    }
