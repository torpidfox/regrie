...
public class Cell implements Serializable {
    ...
    public void resetTSGStatistics() {
        for (TargetSitesGroup ts : tsg.tsg) {
            ts.resetParams();
        }
    }
    ...
    private void generateOutputFilenames(File paramsFilename) throws FileNotFoundException {
        ...
        //this.outputTargetSiteBoundTimeFile =  paramsFilename.getName().replaceAll("params", "ts_time_bound")
        // .replaceAll(Constants.PARAMETR_FILE_EXTENSION, Constants.TF_FILE_EXTENSION);
    ...
    private void createTFSpecies() {
        ...
        TFfileParser TFparser = new TFfileParser(this, this.ip.TF_FILE.value,
                ...
                this.ip.TF_UNCORRELATED_DISPLACEMENT_SIZE.value, this.ip.TF_STALLS_IF_BLOCKED.value,
                this.ip.TF_COLLISION_UNBIND_PROBABILITY.value, this.ip.TF_AFFINITY_LANDSCAPE_ROUGHNESS.value,
                this.ip.TF_PREBOUND_PROPORTION.value, this.ip.TF_PREBOUND_TO_HIGHEST_AFFINITY.value,
                this.ip.TF_IS_IMMOBILE.value, dna.subsequence, this.ip.IS_BIASED_RANDOM_WALK.value,
                this.ip.IS_TWO_STATE_RANDOM_WALK.value, this.ip.REPRESSOR.value, this.ip.TF_REPLENLEFT.value,
                this.ip.TF_REPLENRIGHT.value, this.ip.PWM_REP_THRESHOLD.value, this.ip.REPRESSION_PROBABILITY.value);
        if (TFparser.parsed) {
            // load TF species
                ...
                while (i < TFspecies.length) {
                    ...
                    if (copyNumber > 0) {
                        ...
                        TFspecies[i] = new TFspecies(i, dna.strand, pos, dbdLength, copyNumber, this.ip.TF_ES.value,
                                ...
                                this.ip.TF_STEP_LEFT_SIZE.value, this.ip.TF_STEP_RIGHT_SIZE.value,
                                this.ip.TF_UNCORRELATED_DISPLACEMENT_SIZE.value, this.ip.TF_STALLS_IF_BLOCKED.value,
                                this.ip.TF_COLLISION_UNBIND_PROBABILITY.value,
                                ...
                                this.ip.IS_TWO_STATE_RANDOM_WALK.value, this.ip.REPRESSOR.value,
                                this.ip.TF_REPLENLEFT.value, this.ip.TF_REPLENRIGHT.value,
                                this.ip.PWM_REP_THRESHOLD.value, this.ip.REPRESSION_PROBABILITY.value);
                        moleculesCopyNumber += TFspecies[i].copyNumber;
                        i++;
...
//
//		try {
//			PrintWriter writer = new PrintWriter("/Users/dmitrav/Politech/Laboratory/StoсhasticModelling/reGRiE2/debug
// /ts.txt", "UTF-8");
...
//			}
    ...
    private boolean _everyTSVisited(int min) {
        for (TargetSitesGroup ts : tsg.tsg) {
            if (ts.isAvailable) {
                if (ts.timesReached < min) {
                    return false;
                }
            }
        }
    ...
    public double run() throws FileNotFoundException {
        ...
        while ((this.ip.STOP_TIME.value - cellTime >= doubleZero) && hasNextEvent) {
            //while (continueSimulation && hasNextEvent) {

            //continueSimulation = !_everyTSVisited(100);
...
//			this.printTargetSiteToFollowInfo();
    ...
    private boolean executeNextEvent(boolean fixStopTime) {
        ...
        if (e != null) {
            ...
            if (e instanceof ProteinEvent) {
                ...
                //System.out.println(fixStopTime+" : "+e.time +" : "+this.totalStopTime+" : "+(!fixStopTime || e.time
                // <= this.totalStopTime)+" : "+(fixStopTime && e.time > this.totalStopTime));
                pe = (ProteinEvent) e;
    ...
    public void printSteadyStates(double time) {
        ...
        try {
            ...
            if (false) {
                ...
                for (int i = 0; i < TFspecies.length; i++) {
                    filename = this.outputParamsFile.getName().replaceAll("params", TFspecies[i].name +
                            "_hopping_lengths").replaceAll(Constants.PARAMETR_FILE_EXTENSION,
                            Constants.HOPPING_LENGTHS_FILE_EXTENSION);
                    if (time < this.totalStopTime) {
            ...
            if (false) {
                ...
                for (int i = 0; i < TFspecies.length; i++) {
                    filename = this.outputParamsFile.getName().replaceAll("params", TFspecies[i].name +
                            "_3D_hopping_lengths").replaceAll(Constants.PARAMETR_FILE_EXTENSION,
                            Constants.HOPPING_LENGTHS_FILE_EXTENSION);
                    if (time < this.totalStopTime) {
     ...
     */
    public void updateTargetSiteStatistics(int tsID, double time, boolean bound, double timeBound, int proteinID) {
        //add a line in the target sites follow file with the occupancy up to the last moment the target site was
        // displayed the previous occupancy
			/*if(this.ip.FOLLOW_TS.value){
				String strOld;
...
//				try {
//					//Construct the BufferedWriter object
//					statusBuffer = new BufferedWriter(new FileWriter(new File(this.outputPath,this
// .outputTargetSiteBoundTimeFile), true));
//
//					statusBuffer.write(this.tsg.ts.get(tsID).TFname + " " + this.tsg.ts.get(tsID).relStart + " " +
// timeBound);
//					statusBuffer.newLine();
//					statusBuffer.flush();
//					statusBuffer.close();
