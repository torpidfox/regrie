...
public class EventList  implements Serializable{
	...
	public void scheduleNextTFBindingEvent( Cell n, double time){
		...
		if(n.freeTFmoleculesTotal > 0 && this.TFBindingEventQueue.proteinBindingPropensitySum>0){ 
			...
			//find next reaction
			int nextTFspecies = Gillespie.getNextReaction(this.TFBindingEventQueue.proteinBindingPropensitySum*n.randomGenerator.nextDouble(),
			...
			if(nextTFspecies>Constants.NONE && nextTFspecies<n.TFspecies.length){
				...
				if(TFID != Constants.NONE){
					...
					this.TFBindingEventQueue.add(new ProteinEvent(time+nextTime, TFID, position, true, Constants.EVENT_TF_BINDING, false, false, false));
				}
			}
		}
	}
